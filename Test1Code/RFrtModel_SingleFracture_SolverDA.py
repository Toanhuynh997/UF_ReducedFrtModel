...
#     print(TriMeshes[0].DOFs)
    ...
    for k in range(2)
        condition = (TriMeshes[k].BndryEdge == id_dirichlet[k]) & (nodes[k][edges[k][:, 1], 1] <= 0.2)
    ...
    for k in range(2):
        condition = (
            ...
            ((TriMeshes[k].BndryEdge == id_dirichlet[k]) & (nodes[k][edges[k][:, 1], 1] > 0.2))
        )
...
def DecompSol_AllLocal(sln, TriMesh, ny, node_frt):
    timeshape = sln.shape[0]
    DOFs_Local = [None] * 2
    NumEgs = [None]*2
    NumEms = [None]*2
    ...
    SlnLocal = torch.zeros(timeshape, DOFs_Local[0]+DOFs_Local[1], dtype=torch.float64)
    Slnfrt = torch.zeros(timeshape, ny+node_frt, dtype=torch.float64)
    
    SlnLocal[:, :] += sln[:, :DOFs_Local[0]+DOFs_Local[1]]
    
    Slnfrt[:, :] += sln[:, DOFs_Local[0]+DOFs_Local[1]:DOFs_Local[0]+DOFs_Local[1]+node_frt+ny]
              
    return SlnLocal, Slnfrt
...
def ComputSol_DFFracture_Afrt_Ksubdom_DA(
